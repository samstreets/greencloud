...
read -r API_KEY
echo
if ! /wfs/bin/gccli login -k "$API_KEY"  >/dev/null 2>&1; then
...
if /wfs/bin/gccli node add --external --id "$NODE_ID" --description "$NODE_NAME" >/dev/null 2>&1; then
  echo -e "${GREEN}✔ Node added successfully!${NC}"
else
  echo -e "${YELLOW}Failed to add node via gccli. Please retry manually:${NC}"
  echo "/wfs/bin/gccli node add --external --id $NODE_ID --description \"$NODE_NAME\""
  exit 1
fi
